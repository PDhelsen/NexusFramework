...
namespace NxFr
{
	...
	{
		NX_ASSERT(Months > 0 && Months <= 12, Default, "Invalid Months");
		NX_ASSERT(Days > 0 && Days <= GetDaysPerMonth(), Default, "Invalid Days");
		NX_ASSERT(Hours >= 0 && Hours < 24, Default, "Invalid Hours");
		NX_ASSERT(Minutes >= 0 && Minutes < 60, Default, "Invalid Minutes");
		NX_ASSERT(Seconds >= 0 && Seconds < 60, Default, "Invalid Seconds");
	}
