...
namespace NxEn
{
	...
	namespace Hashing
	{
		...
		public:
			...
			NEXUS_ENGINE_API HashAlgorithm(HashLength Seed) {};

			NEXUS_ENGINE_API virtual HashAlgorithm& Accumulate(const void* Data, uint64 Length) = 0;
			NEXUS_ENGINE_API virtual HashLength Hash() const = 0;

			NEXUS_ENGINE_API virtual HashLength Combine(HashLength HashA, HashLength HashB) const = 0;
		...
		public:
			...
			NEXUS_ENGINE_API HashLength Combine(HashLength HashA, HashLength HashB) const override;
	...
	template<typename H = Hashing::HashAlgorithmDefault>
	struct HashStrategy
	{
	public:
	...
	template<typename T = void*, typename H = Hashing::HashAlgorithmDefault>
	class HashProcess
	{
	public:
	...
	public:
		...
		{
			HashStrategy<H> State(Seed);
			return HashProcess<T, H>::Hash(State, Data);
		}
